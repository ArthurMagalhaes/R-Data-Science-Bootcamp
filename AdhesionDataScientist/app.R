...
ui <- fluidPage(
   ...
   fluidRow(
     ...
     column(6, actionButton("Processar", "Processar"), 
            helpText("A linha vermelha mostra o grau de aderência à carreira de Cientista de Dados."), 
...
server <- function(input, output) {
   ...
   inferencia = fuzzy_inference(sistema, list(Gosta_exatas = input$sexatas, Rel_interpess = input$sinter, Escre_codigo = input$scodigo, 
                                              Perfil_lider = input$slider, Gosta_Estudar = input$sestudar, Habilidade_comunicao = input$scomunicao))
